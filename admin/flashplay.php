...
if ($_REQUEST['act']== 'list')
{
    ...
    $smarty->assign('action_link_makexml', array('text' => $_LANG['ads_make_xml'], 'href' => 'flashplay.php?act=makexml'));//Add by King, 生成 xml 的按钮
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    if (empty($_POST['step']))
    {
        ...
        $src_bg = isset($_GET['src_bg']) ? $_GET['src_bg'] : '';
        $sort = 0;
        $rt = array('act'=>'add','img_url'=>$url,'img_src'=>$src,'img_bg_src'=>$src_bg, 'img_sort'=>$sort);
    ...
    elseif ($_POST['step'] == 2)
    {
		...
		if (!empty($_FILES['img_bg_file_src']['name']))
        {
            if(!get_file_suffix($_FILES['img_bg_file_src']['name'], $allow_suffix))
            {
                sys_msg($_LANG['invalid_type']);
            }
			$name = str_replace(strrchr($name, "."),"",$name);
            $name .=  '_bg.' . end(explode('.', $_FILES['img_bg_file_src']['name']));
            $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;
            if (move_upload_file($_FILES['img_bg_file_src']['tmp_name'], $target))
            {
                $src_bg = DATA_DIR . '/afficheimg/' . $name;
            }
        }
		elseif (!empty($_POST['img_bg_src']))
        {
            $src_bg = $_POST['img_bg_src'];

            if(strstr($src_bg, 'http') && !strstr($src_bg, $_SERVER['SERVER_NAME']))
            {
                $src_bg = get_url_image($src_bg);
            }
        }
        ...
        // 插入新数据
        array_unshift($flashdb, array('src'=>$src, 'url'=>$_POST['img_url'], 'text'=>$_POST['img_text'] ,'sort'=>$_POST['img_sort'], 'src_bg'=>$src_bg));
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    elseif ($_POST['step'] == 2)
    {
		...
		if (!empty($_FILES['img_bg_file_src']['name']))
        {
            ...
            //有上传
//			$name_l = '/'. DATA_DIR . '\/afficheimg\//';
			$name = preg_replace('/'. DATA_DIR . '\/afficheimg\//','',$rt['src']);
            $name = str_replace(strrchr($name, "."),"",$name);
            $name .=  '_bg.' . end(explode('.', $_FILES['img_bg_file_src']['name']));
            $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;
		...
		else if (!empty($_POST['img_bg_src']))
        {
            $src_bg =$_POST['img_bg_src'];
        ...
        $flashdb[$id] = array('src'=>$src,'url'=>$_POST['img_url'],'text'=>$_POST['img_text'],'sort'=>$_POST['img_sort'],'kind'=>$_POST['img_kind'],'src_bg'=>$src_bg);
...
//Add by King, 首页 flash 生成 xml 文件
elseif ($_REQUEST['act'] == 'makexml')
{
    $xml = '<?xml version="1.0" encoding="' . EC_CHARSET . '"?>'."\n".'<photo>';
    $playerdb = get_flash_xml();
    foreach ($playerdb as $key => $val)
    {
        $xml .= "\n\t".'<pic photo="' . $uri . '' . $val['src'] . '" lianjie="' . $val['url'] . '" kind="' . $val['kind'] . '" />';
    }
    $xml .= "\n".'</photo>';
    file_put_contents(ROOT_PATH . DATA_DIR . '/flash_index.xml', $xml);

    clear_cache_files(); // 清除模版缓存

    $links[] = array('text' => $_LANG['go_url'], 'href' => 'flashplay.php?act=list');
    sys_msg($_LANG['ads_make_xml'], 0, $links);

}
//End Add by King, 首页 flash 生成 xml 文件
...
function get_flash_xml()
{
    ...
    if (file_exists(ROOT_PATH . DATA_DIR . '/flash_data.xml'))
    {
        ...
        // 兼容v2.7.0及以前版本
        if (!preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssort="([^"]*)"\skind="([^"]*)"\ssrc_bg="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . '/flash_data.xml'), $t, PREG_SET_ORDER))
        {
        ...
        if (!empty($t))
        {
            ...
            {
                $val[6] = isset($val[6]) ? $val[6] : 0;
                $flashdb[] = array('src'=>$val[1],'url'=>$val[2],'text'=>$val[3],'sort'=>$val[4],'kind'=>$val[5],'src_bg'=>$val[6]);
            }
        }
    }
    return $flashdb;
...
function put_flash_xml($flashdb)
{
    if (!empty($flashdb))
    {
        ...
        foreach ($flashdb as $key => $val)
        {
            $xml .= '<item item_url="' . $val['src'] . '" link="' . $val['url'] . '" text="' . $val['text'] . '" sort="' . $val['sort'] . '" kind="' . $val['kind'] . '" src_bg="' . $val['src_bg'] . '"/>';
        }
        $xml .= '</bcaster>';
