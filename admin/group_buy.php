...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($_REQUEST['act'] == 'add')
    {
        $group_buy = array(
            'act_id'  => 0,
            'start_time'    => date('Y-m-d H:i', time() + 86400),
            'end_time'      => date('Y-m-d H:i', time() + 4 * 86400),
	...
	/************ Add by King ,copy the group buy ***************/
	else if ($_REQUEST['act'] == 'copy')
	{
		$group_buy_id = intval($_REQUEST['id']);
		if ($group_buy_id <= 0)
        {
		...
		$sql = "SELECT * FROM " . $ecs->table('goods_activity') . " WHERE act_id = '$group_buy_id'";
        $groupon = $db->getRow($sql);
		$group_buy['act_id'] = '';
		$group_buy['goods_name'] = $group_buy['goods_name'];
		$group_buy['start_time'] = local_date('Y-m-d H:i', $groupon['start_time'] + (1 * 24 * 60 * 60));
		$group_buy['end_time']   = local_date('Y-m-d H:i', $groupon['end_time'] + 1*86400);
		$smarty->assign('copy_next_id', $_REQUEST['id']+1);//Add by King, $_REQUEST['act'] , $_REQUEST['id'] 当前链接中产品 id 值
	
	}
	/************* End by King ,copy the group buy **************/
	$smarty->assign('current_link', $_REQUEST);//Add by King, $_REQUEST['act'] , $_REQUEST['id'] 当前链接中产品 id 值
...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    elseif (isset($_POST['fail']))
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods_activity') .
                " SET is_finished = '" . GBS_FAIL . "', " .
                    ...
                    "groupon_desc = '$_POST[groupon_desc]' " .//Add by King
                "WHERE act_id = '$group_buy_id' LIMIT 1";
        $db->query($sql);
    ...
    else
    {
        ...
        $info = goods_group_buy($goods_id);//得到一条此商品的团购信息，下面的判断为，如果有一件商品参加团购，则此商品不允许再有团购，因此后面的团购要重新编辑也无法进行编辑
		/* 注释掉, 让此判断失效，同以商品，可以在不同时间举行团购。
        if ($info && $info['act_id'] != $group_buy_id)
        {
            sys_msg($_LANG['error_goods_exist'].'----'.$info['act_id'].'----'.$group_buy_id);//edit by King
        }
		*/
        ...
        $group_buy = array(
            'act_name'   => $act_name,
            'act_desc'   => $_POST['act_desc'],
            'act_groupon_desc'   => $_POST['groupon_desc'],//Add by King
        ...
        else
        {
			...
			$groupon_id = mysql_insert_id();//add by King ,get the last cat_id
            ...
            /* 提示信息 */
            $links = array(
                array('href' => 'group_buy.php?act=copy&id='.$groupon_id, 'text' => $_LANG['copy_success']),//Add by King
...
//        $cur_amount = $stat['valid_goods'];         // 当前数量 //实现购买数量达到多少时，所有人可享受更优惠价格。不用这个，只有个人购买一定数量时才有优惠价格。edit by King
		$cur_amount = 0;	//当前数量为0，则大家都以相同价格购买
        ...
        $arr['start_time']  = local_date('Y-m-d H:i', $arr['start_time']);//edit by King //$GLOBALS['_CFG']['date_format']
        $arr['end_time']    = local_date('Y-m-d H:i', $arr['end_time']);//edit by King //$GLOBALS['_CFG']['date_format']
