...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    $smarty->assign('child_categories', get_categories_tree($cat_id)); // 分类树
    $smarty->assign('helps',            get_shop_help());              // 网店帮助
    $smarty->assign('top_goods',        get_top10());                  // 销售排行
    $smarty->assign('show_marketprice', $_CFG['show_marketprice']);
    $smarty->assign('category',         $cat_id);

	$parent_info = get_cat_info($cat_id);//edit by King
	$smarty->assign('parent_id',        $parent_info['parent_id']);//edit by King
...
//print_r(assign_cat_goods(3,50));exit;
//print_r (get_attribute_valuea(38,219));exit;
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.goods_ename, g.goods_ename_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_number, g.goods_desc, g.goods_thumb , g.goods_img ' .//Add the  goods_desc and goods_number by king
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE $where $ext ORDER BY g.sort_order DESC, $sort $order";//edit by King 使分类下商品列表按照后台推荐排序号降序优先排列
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['ename']            = $row['goods_ename'];//取得属性名称和值 Add by King
        $arr[$row['goods_id']]['goods_brief']      = $row['goods_brief'];
        $arr[$row['goods_id']]['goods_number']     = $row['goods_number'];//Add by king
        $arr[$row['goods_id']]['goods_desc']	   = preg_replace ( "/(\<[^\<]*\>|\r|\n|\s|\[.+?\])/is", ' ', $row['goods_desc'] );//htmlspecialchars_decode($row['goods_desc']);//Add by king
		$arr[$row['goods_id']]['goods_desc_txt']   = strip_tags ( $row['goods_desc'],"<br><p>" );//Add by king
        $arr[$row['goods_id']]['goods_style_name'] = add_style($row['goods_name'],$row['goods_name_style']);
        $arr[$row['goods_id']]['goods_style_ename'] = add_style($row['goods_ename'],$row['goods_ename_style']);//取得属性名称和值 Add by King
        $arr[$row['goods_id']]['market_price']     = price_format($row['market_price']);
        $arr[$row['goods_id']]['shop_price']       = price_format($row['shop_price']);
        $arr[$row['goods_id']]['shop_price_n']     = $row['shop_price'];//只取商城价格的数字，不加文字 Add by King
		...
		//Add by King
		$arr[$row['goods_id']]['att']			   = get_attribute_valuea($row['goods_id'],219);//取得属性名称和值 Add by King
		$arr[$row['goods_id']]['attime']		   = get_attribute_valuea($row['goods_id'],172);//取得属性名称和值 Add by King
		//End by King
...
//Add by King
 ...
 function get_attribute_valuea($goods_id, $attri_id = '')
{
    /* 对属性进行重新排序和分组 */
