...
/* the time reset, add by King */

			$gb_list = group_buy_list_all($size, $page);
...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    else
    {
        ...
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent, $goods->shipDate))//Add by King
        {
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	/* 取得购物车中商品要求送货时间 Add by King */
    $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
    /* 对商品信息赋值 */
    $cart_goods = cart_goods($flow_type);
	/* 此处暂时没用到，留以备用 end by King */
...
elseif($_REQUEST['step'] == 'update_cart_ajax')
{
    ...
    //$result['content'] = insert_cart_info();    //更新購物車區塊 (若你的購物流程頁面沒有包含 {insert name='cart_info'} 的話，則不需要放這段)
    die($json->encode($result));
}
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy')
        {
            ...
            ******************/
            ...
            if (!empty($goods['product_id']))
            {
                ...
                {
                    $result['goods_number'] = $goods['goods_number'];
                    $result['content'] =sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'],$product_number['goods_number'], $product_number['goods_number']);
                    return $result;
