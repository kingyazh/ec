...
//               'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = ' . $goods_id . ' ORDER BY oi.add_time DESC LIMIT ' . (($page > 1) ? ($page-1) : 0) * 5 . ',5';
               'WHERE oi.order_id = og.order_id AND og.goods_id = ' . $goods_id . ' ORDER BY oi.add_time DESC LIMIT ' . (($page > 1) ? ($page-1) : 0) * 5 . ',5';//edit by King
...
//               'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = ' . $goods_id;
               'WHERE oi.order_id = og.order_id AND og.goods_id = ' . $goods_id;//eidt by King
        $count = $db->getOne($sql);
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('child_categories',   get_categories_tree($goods['cat_id']));  // 当前分类下的子分类树 add by King
        ...
        $smarty->assign('pictures_mensa',      get_goods_gallery_mensa($goods_id));              // Add by King, mensa 商品相册
...
//print_r (get_goods_gallery_mensa($goods_id));exit;//Add by King
...
function get_linked_goods($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['goods_ename']  = $row['goods_ename'];
...
{
    $sql = 'SELECT COUNT(b.goods_id ) AS num, g.goods_id, g.goods_name, g.goods_ename, g.goods_thumb, g.goods_img, g.shop_price, g.promote_price, g.promote_start_date, g.promote_end_date ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$key]['goods_ename'] = $row['goods_ename'];
