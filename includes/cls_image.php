...
class cls_image
{
    ...
    function cls_image($bgcolor='')
    {
        ...
        {
//            $this->bgcolor = "#FFFFFF";
//            $this->bgcolor = "#dbcea7";//eidt by King
            $this->bgcolor = "";//eidt by King, 得到透明背景
        }
    }
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
    {
		...
		imagealphablending($img_org,false);//这里很重要,意思是不合并颜色,直接用$img图像颜色替换,包括透明色;//Add by King
		imagesavealpha($img_org,true);//这里很重要,意思是不要丢了$thumb图像的透明色;//Add by King
        ...
        if ($org_info[0] / $thumb_width > $org_info[1] / $thumb_height)
        {
            ...
            $lessen_height  = $thumb_width;// / $scale_org;//edit by King
        }
        else
        {
            /* 原始图片比较高，则以高度为准 */
            $lessen_width  = $thumb_height;// * $scale_org;//edit by King
        ...
        /* 将原始图片进行缩放处理 */
		imagealphablending($img_thumb,false);//这里很重要,意思是不合并颜色,直接用$img图像颜色替换,包括透明色;//Add by King
		imagesavealpha($img_thumb,true);//这里很重要,意思是不要丢了$thumb图像的透明色;//Add by King
        if ($gd == 2)
        {
...
//            imagejpeg($img_thumb, $dir . $filename);
            imagejpeg($img_thumb, $dir . $filename, 90);//edit by King
        }
