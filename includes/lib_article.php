...
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='')
{
    ...
    if ($requirement != '')
    {
        $sql = 'SELECT article_id, title, author, add_time, file_url, open_type, content' .//edit by King , add content
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
			...
			$article_file_name    = pathinfo($row['file_url']);//add by King
            ...
            $arr[$article_id]['true_url']    = build_uri('article', array('aid'=>$article_id), $row['title']);//add by King
            $arr[$article_id]['file_url']    = $article_file_name['extension'] == 'jpg' || $article_file_name['extension'] == 'gif' || $article_file_name['extension'] == 'png' ? trim($row['file_url']) : '';//add by King
