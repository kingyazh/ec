    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $cat_arr[$row['cat_id']]['parent']   = $row['parent_id'];//Add by King
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
               ...
               $three_arr[$row['cat_id']]['parent']   = $row['parent_id'];//Add by King
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.goods_ename, g.goods_ename_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['ename']        = $row['goods_ename'];
            $goods[$idx]['brief']        = $row['goods_brief'];
            $goods[$idx]['brand_name']   = isset($goods_data['brand'][$row['goods_id']]) ? $goods_data['brand'][$row['goods_id']] : '';
            $goods[$idx]['goods_style_name']   = add_style($row['goods_name'],$row['goods_name_style']);
            $goods[$idx]['goods_style_ename']  = add_style($row['goods_ename'],$row['goods_ename_style']);
			...
			$goods[$idx]['att']			 = get_attribute_value($row['goods_id'],166);//取得属性名称和值 Add by King
			$goods[$idx]['goods_pics']	 = get_goods_gallery($row['goods_id']);//取得当前商品的相册 Add by King
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		/* 取去掉 html 标签的商品描述，保留某个标签可选。Add by King */
		$row['goods_desc_txt']	 = strip_tags ( str_replace ( " ", '', $row['goods_desc'] ),"<br><p>" );//Add by king
...
function get_goods_properties($goods_id)
{
    ...
    /* 获得商品的规格 */
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, a.attr_input_type, ".
    ...
    foreach ($res AS $row)
    {
        ...
        if ($row['attr_type'] == 0)
        {
            ...
            $arr['pro'][$group][$row['attr_id']]['attr_input_type'] = $row['attr_input_type'];
        }
        else
        {
...
function get_same_attribute_goods($attr)
{
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            /* 查找符合条件的商品 */
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_ename, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                        "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
            ...
            foreach ($res AS $row)
            {
                ...
                $lnk[$key]['goods'][$row['goods_id']]['goods_ename']   = $row['goods_ename'];
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_ename, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_number, g.goods_desc, g.goods_thumb, g.goods_img ' .//Add the  goods_desc and goods_number by king
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    ...
    $sql .= $order_rule;
//    $sql .= $order_rule . ' , g.sort_order DESC ';//edit by King
    if ($num > 0)
    {
        $sql .= ' LIMIT ' . $num;
    }
    ...
    foreach ($res AS $idx => $row)
    {
		...
		$goods[$idx]['goods_number']     = $row['goods_number'];//Add by king
        $goods[$idx]['goods_desc']		 = preg_replace ( "/(\<[^\<]*\>|\r|\n|\s|\[.+?\])/is", ' ', $row['goods_desc'] );//htmlspecialchars_decode($row['goods_desc']);//Add by king
        $goods[$idx]['goods_desc_txt']	 = strip_tags ( $row['goods_desc'],"<br><p>" );//Add by king
    ...
    $cat['id']   = $cat_id;
//print_r ($goods[$idx]);exit;//testing by King
...
function group_buy_info($group_buy_id, $current_num = 0)
{
    ...
    else
    {
        foreach ($price_ladder as $key => $amount_price)
        {
            ...
            $price_ladder[$key]['easy_price'] = $amount_price['price'];
            $price_ladder[$key]['total_price'] = $amount_price['price']*$amount_price['amount'];
        }
    }
...
//    $cur_amount = $stat['valid_goods'] + $current_num; // 当前数量 // eidt by King //此处实现购买数量达到多少时，所有人可享受更优惠价格。不用这个，只有个人购买一定数量时才有优惠价格。edit by King
    ...
    $group_buy['end_time'] = $group_buy['formated_end_date'];
		...
		/* 购物车显示单位 修改二 start, eidt by King */
		$sql="select measure_unit from ".$GLOBALS['ecs']->table('goods')." g left join " . $GLOBALS['ecs']->table("category") . "c on g.cat_id=c.cat_id where goods_id=". $group_buy['goods_id'] ." ";
        $measure_unit=$GLOBALS['db']->getOne($sql);
        $group_buy['measure_unit']=$measure_unit;
		/* 购物车显示单位 修改二 end, eidt by King */

	/* 商品详细信息 */
	/* get the goods info, start by King */
	$group_buy['goods_desc'] = $GLOBALS['db']->getOne("select goods_desc from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$group_buy['goods_id']);//add by King
	/* get the goods info, end by King */
 ...
 * @return  array
 */
function group_buy_link()
{
    $now = gmtime();
	$tomrrow_time = strtotime(date(local_date('Y-m-d',gmtime())))+16*3600-1;           // 第二天 0 点之前，大概为23:59:59// add by King
	$this_week_monday = this_monday(gmtime());//获取当前时间的本周周一的时间 // Add by King

	$sql="select act_id from ".$GLOBALS['ecs']->table('goods_activity')." where".
	" end_time >= $this_week_monday AND end_time <= $tomrrow_time ORDER BY end_time DESC";
	$group_buy_id=$GLOBALS['db']->getOne($sql);
	$new_group_link= "group_buy.php?act=view&amp;id=".$group_buy_id;
