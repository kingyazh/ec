...
function insert_bought_notes($arr)
{
    ...
    /* 商品购买记录 */
if($arr['id'] == ''){
    $sql = 'SELECT u.user_name, og.goods_name, og.goods_number, og.goods_attr, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           ...
           'WHERE oi.order_id = og.order_id ORDER BY oi.add_time DESC LIMIT 6';//eidt by King
}else{
    $sql = 'SELECT u.user_name, og.goods_name, og.goods_number, og.goods_attr, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           'FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' .
//           'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND ( og.goods_id = ' . $arr['id'] . ' ) ORDER BY oi.add_time DESC LIMIT 5';
           'WHERE oi.order_id = og.order_id AND og.goods_id = ' . $arr['id'] . ' ORDER BY oi.add_time DESC LIMIT 6';//eidt by King
}
    ...
    foreach ($bought_notes as $key => $val)
    {
        ...
        $bought_notes[$key]['add_time_easy'] = local_date("Y-m-d", $val['add_time']);
        $bought_notes[$key]['goods_attr'] = sub_str($val['goods_attr'] ? $val['goods_attr'] : '',4,false).'';
...
//           'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND ( og.goods_id = ' . $arr['id'] .' )';
           'WHERE oi.order_id = og.order_id';//eidt by King
}else{
    $sql = 'SELECT count(*) ' .
           'FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' .
//           'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND ( og.goods_id = ' . $arr['id'] .' )';
           'WHERE oi.order_id = og.order_id AND og.goods_id = ' . $arr['id'] ;//eidt by King
}
