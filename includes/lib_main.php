...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    /* 初始化“页面标题”和“当前位置” */
    $page_title = $GLOBALS['_CFG']['shop_title'] . ' - ' . 'PinMeiSiJi';
                        ...
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    ...
    switch ($app)
    {
        ...
        break;
		//end the index page,edit by King
...
function assign_comment($id, $type, $page = 1)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['comment_id']]['add_date'] = local_date('Y-m-d', $row['add_time']);//eidt by King
    }
    /* 取得已有回复的评论 */
    if ($ids)
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($res))
        {
            ...
            $arr[$row['parent_id']]['re_add_date'] = local_date('Y-m-d', $row['add_time']);//eidt by King
	...
	$pager['page_number'] = array();
            for ($i=1;$i<=$page_count;++$i)
            {
                $pager['page_number'][$i] = "javascript:gotoPage($i,$id,$type)";
            }
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('new_group_goods',		 group_buy_link());  //最新团购的链接，可以做最新活动的链接，查找函数，改数据库查询操作
