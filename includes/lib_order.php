...
function cart_goods($type = CART_GENERAL_GOODS)
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            "market_price, goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, ship_date, is_shipping, " .//edit by King
    ...
    foreach ($arr as $key => $value)
    {
		...
		/* 购物车显示单位 修改一 start, edit by King */
		$sql="select measure_unit from " . $GLOBALS['ecs']->table('goods') . "g left join " . $GLOBALS['ecs']->table("category") . " c on g.cat_id=c.cat_id where goods_id=" . $value['goods_id'] ." ";
        $measure_unit=$GLOBALS['db']->getOne($sql);
        $arr[$key]['measure_unit']=$measure_unit;

		/* 购物车显示单位 修改一 end, edit by King */
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $shipDate)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    ...
    $parent = array(
        ...
        'is_gift'       => 0,
        'ship_date'     => gmtime()-date('O')*72-gmtime()%86400+2*86400, //Add by King
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods_storage)
            {
                ...
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number = '$num'" .
                       ...
                       " , ship_date = '$shipDate'".//Add by King , 更新预定时间
                       " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$goods_id' ".
                       " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                       " AND extension_code <> 'package_buy' " .
                       "AND rec_type = 'CART_GENERAL_GOODS'";
        ...
        else //购物车没有此物品，则插入
        {
            ...
            $parent['parent_id']    = 0;
            $parent['ship_date']    = $shipDate;//Add by King,预定时间
...
function get_goods_attr_info($arr, $type = 'pice')
{
    ...
    if (!empty($arr))
    {
        //$fmt = "%s:%s[%s] \n";//购物车中商品属性，以此种格式进入数据库，再用 get_cart_goods() 从数据库中读出，edit by King
		$fmt = "%s \n";//购物车中商品属性，edit by King
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            //$attr .= sprintf($fmt, $row['attr_name'], $row['attr_value'], $attr_price);//购物车中商品属性，edit by King
            $attr .= sprintf($fmt, $row['attr_value']);//购物车中商品属性，edit by King
        }
