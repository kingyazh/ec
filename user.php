...
'account_deposit', 'account_log', 'account_detail', 'points_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
...
/****************** 登录处理，所有页面都要显示个人详细信息 add by King **/
else{
...
if ($action == 'default')
{
	//此处已重复调用，已将此调用信息放置用户中心全局 edit by KIng
    ...
    $smarty->assign('prompt',      get_user_prompt($user_id));//print_r($consignee_list);exit;
...
elseif ($action == 'points_detail')
{
    ...
    //获取剩余余额
    $points_amount = get_user_points($user_id);
	...
	//获取积分等级
    $rank_points = array();
    $sql = "SELECT * FROM " . $ecs->table('user_rank') .
           //" WHERE user_id = '$user_id'" .
           //" AND $account_type <> 0 " .
           //" ORDER BY log_id DESC";
           " ORDER BY min_points, max_points ASC";
    //$res = $GLOBALS['db']->selectLimit($sql, $pager['size'], $pager['start']);
	$res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        $row['rank_name'] = $row['rank_name'];
        $row['min_points'] = $row['min_points'];
        $row['max_points'] = $row['max_points'];
        $row['discount'] = $row['discount'];
        $row['show_price'] = $row['show_price'];
        $row['special_rank'] = $row['special_rank'];
        $rank_points[] = $row;
    }

    //模板赋值
    $smarty->assign('points_amount',  $points_amount);
    $smarty->assign('account_log',    $account_log);
    $smarty->assign('rank_points',    $rank_points);//Add by King
    $smarty->assign('pager',          $pager);
    $smarty->display('user_transaction.dwt');
}
/* end by King */
